...
export const FormSection: React.FC<FormSectionProps> = ({ title, icon, children, containerClassName = '' }) => {
  return (
    <section className="flex flex-col gap-2.5 w-full">
      ...
      <div className={`bg-white p-5 sm:p-7 rounded-[24px] sm:rounded-[28px] border border-slate-100 shadow-[0_4px_20px_-10px_rgba(0,0,0,0.04)] flex flex-col gap-5 sm:gap-6 ${containerClassName}`}>
        {children}
      </div>
    </section>
  );
