...
const App = () => {
  // App.tsx тепер бере лише isPro для водяного знаку. Решта йому не потрібна!
  const { isPro } = usePro(); 
  const { t } = useLanguage();
  ...
  return (
    <div className="app-shell bg-[#F8FAFC] min-h-screen relative overflow-x-hidden flex flex-col">
      ...
      {currentView === 'editor' ? (
        <main className="flex-1 flex flex-col items-center justify-center p-4 sm:p-8 min-h-[calc(100vh-80px)] pb-32">
      ...
      {currentView === 'editor' && !isSettingsOpen && (
        ...
        >
          ...
          {t.toolbar || 'Панель інструментів'}
        </button>
      )}

      {currentView === 'editor' && (
        <FloatingPanel isOpen={isSettingsOpen} onClose={() => setIsSettingsOpen(false)} title={t.toolbar || 'Панель інструментів'}>
          <AiToolsSection 
      ...
      {/* Модалка тепер рендериться без пропсів, бо бере все з Контексту */}
      <SubscriptionModal />
